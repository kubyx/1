...
class ProgramLauncher:
    ...
    def create_program_shortcut(self, program_name, target_path, icon_path=None):
        """Program kısayolu oluştur"""
        try:
            # Basit .lnk oluşturma (gerçek uygulamada win32com kullanılabilir)
            import winshell
            ...
            shortcut.save()
            logging.info(f"Kısayol oluşturuldu: {shortcut_path}")
            return True
            
        except Exception as e:
            logging.error(f"Kısayol oluşturma hatası: {e}")
            return False
